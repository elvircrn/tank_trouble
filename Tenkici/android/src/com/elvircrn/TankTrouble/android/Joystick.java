    ...
    public void update(float deltaTime) {
        ...
        for (int i = 0; i < Input.count(); i++) {
            if (inputRegion.contains(Input.get(i))) {
                ...
                else if ((Input.get(i).x - analog.center.x) * (Input.get(i).x - analog.center.x) +
                         (Input.get(i).y - analog.center.y) * (Input.get(i).y - analog.center.y ) < Analog.maxDistanceMult * Analog.maxDistanceMult * analog.moveRadius * analog.moveRadius) {
                    foundAnalog = true;
